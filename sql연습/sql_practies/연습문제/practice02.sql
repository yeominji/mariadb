-- 문제 1.최고임금(salary)과 최저임금을 “최고임금, “최저임금”프로젝션 타이틀로 함께 출력해보세요.
...
-- 예) 2014년 07월 10일
select date_format(max(hire_data),'%Y년 %m월%d일')  from employees;
-- 문제3. 가장 오래 근속한 직원의 입사일은 언제인가요? 다음 형식으로 출력해주세요.
-- 예) 2014년 07월 10일
select date_format(min(hire_data),'%Y년 %m월%d일')  from employees;
