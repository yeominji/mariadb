...
select concat(first_name ,'' ,last_name) as 이름 ,emp_no 
...
select count(*)   from employees where gender ='F'and gender='M'; 
-- 문제4.현재(where to _date ='9999-01-01' 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.)
select count(*)   from salaries where to_date='9999-01-01';

-- 문제5.부서는 총 몇 개가 있나요?
select count(*)   from departments; 

-- 문제6 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외, 현재 매니저)
...
select count(*) from titles where title ='Enginner' and  to_date ='9999-01-01' ;
-- 문제11사 번이 13250(Zeydy)인 지원이 직책 변경 상황을 시간순으로 출력해보세요.
select title ,to_date, from_date  from titles where emp_no=13250 order by from_date asc;
